...
const makeSizeMatch = (main, length, char=' ') => {
	if (main.length < length) main = main + char.repeat(length - main.length);
	return main;
}
std.writeTable = (table=[[],[],[]]) => {
	let largest = 0;
	table = table.map(row=>{
	...
	for (let row of table) {
		for (let c = 0; c<row.length; c++) {
			let column = row[c];
			if (c < row.length-1) { base.write(makeSizeMatch(column, largest, ' ')); }
			else { base.write(' '.repeat(overall - state.width - column.length) + column); };
		}
		base.write('\n');
	}
}
